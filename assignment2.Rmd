...
Review: The accuracy on the training data dropped to 0.86. This loss of accuracy does bring the metric much closer to the accuracy on the testing set, signalling less overfitting though. Overall, the random forest algorithm may not be the best choice for this data set.
...
Evaluation: We will continue to use a confusion matrix for accuracy

Experiment:

```{r xg1-mod}
# convert the target column from factor to numeric label
bank_train2 <- bank_train
bank_train2$y <- as.numeric(bank_train2$y)
bank_test2 <- bank_test
bank_test2$y <- as.numeric(bank_test2$y)
...
# convert the data to a matrix, leaving out the final column
...
xg1_train_pred <- factor(xg1_train_pred, levels = 0:1, labels = levels(bank$y))

xg1_train_cm <- confusionMatrix(xg1_train_pred, factor(bank_train2$y, levels = 0:1))
xg1_train_cm
```
# predict and evaluate on testing data
xg1_test_pred <- predict(bank_xg1, test_matrix, type = "class")
xg1_test_cm <- confusionMatrix(xg1_test_pred, test_trsf$y.no)
xg1_test_accuracy <- xg1_test_cm$overall["Accuracy"]
xg1_test_accuracy
```
