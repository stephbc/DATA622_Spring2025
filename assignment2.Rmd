...
The key is to modify only one or a few variables at a time to isolate the impact of each change and understand its effect on model performance.

```{r import}
bank_raw <- read.csv2(file="bank+marketing/bank/bank-full.csv")

bank <- bank_raw |>
  select(-c(contact, day, month))
...
The `contact`, `day` and `month` columns contained somewhat redundant information and so were dropped. The "unknown" and "other" values in `poutcome` were converted to `NA`s for elucidation of the previous campaign's results.
...
Evaluation: A simple confusion matrix can be produced to view predicted values against the actual values, then used to calculate the predictive accuracy.
...
dt1_train_cm <- confusionMatrix(dt1_train_pred, as.factor(bank_train$y))
dt1_train_accuracy <- dt1_train_cm$overall["Accuracy"]
dt1_train_accuracy

# predict and evaluate on testing data
dt1_test_pred <- predict(bank_dt1, bank_test, type = "class")
dt1_test_cm <- confusionMatrix(dt1_test_pred, as.factor(bank_test$y))
dt1_test_accuracy <- dt1_test_cm$overall["Accuracy"]
dt1_test_accuracy
```

Review: The accuracy of this first model is high at just over 90%, and the feature determined to be most predictive of `y` is `duration`. If the last contact with the customer was recorded at under 522 seconds, the probability that the customer has not subscribed to a term deposit is only 8%. It may be interesting to note, this decision tree only chose features related to marketing as most predictive: `duration`, `poutcome` and `pdays`.
...
Objective: To see how the predictive power changes once the data selection is changed. 
...
dt2_train_cm <- confusionMatrix(dt2_train_pred, as.factor(bank_sub_train$y))
dt2_train_accuracy <- dt2_train_cm$overall["Accuracy"]
dt2_train_accuracy

# predict and evaluate on testing data
dt2_test_pred <- predict(bank_dt2, bank_sub_test, type = "class")
dt2_test_cm <- confusionMatrix(dt2_test_pred, as.factor(bank_sub_test$y))
dt2_test_accuracy <- dt2_test_cm$overall["Accuracy"]
dt2_test_accuracy
```

Review: The accuracy is slightly lower but still quite high, and this tree appears similar to the first experiment (low variance). Also, since the difference between the accuracy measures is quite small, it does not appear to be overfitting. However, only marketing-related features were once again chosen by the model.
